<?php 
	...
	$post_query_result = mysqli_query($db, "SELECT * FROM topics WHERE id=1");
	$topic = mysqli_fetch_assoc($post_query_result);

	// Get all comments from database
	$comments_query_result = mysqli_query($db, "SELECT * FROM posts WHERE post_id=" . $post['id'] . " ORDER BY created_at DESC");
	...
	{
		global $db;
		$result = mysqli_query($db, "SELECT user_name FROM users WHERE id=" . $id . " LIMIT 1");
		// return the username
