...
class LoopObject:
    ...
    def transpile(self):
        ...
        if str(keyword) == "for":
            self.exec_str += "for "+ name + " in range("+ str(start_value) + ", " + str(end_value) + ", " + str(increment) + "):"+ self.transpile_scope(scope, self.nesting_count, 5)

        if str(keyword) == 'while':
            self.exec_str += "while "+ name + " " + condition + ":" + self.transpile_scope(scope, self.nesting_count, 4)
    ...
    def transpile_scope(self, body_ast, nesting_count, items):
        ...
        for ast in body_ast:
            ...
            if self.check_ast('conditional_statment', ast):
